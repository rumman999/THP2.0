<template>
  <section class="bg-light vh-100 d-flex align-items-center justify-content-center">
    <div class="container text-center">
      <!-- Title and Description -->
      <h2 class="mb-4">What Makes Us Different?</h2>
      <p class="mb-5">
        At Token of Power Properties Ltd., we combine innovation, meticulous attention to detail, and sustainable design to create spaces that stand the test of time.
      </p>

      <div class="row align-items-center justify-content-center">
        <!-- Left Image -->
        <div class="col-md-6 mb-4 mb-md-0">
          <img
            src="@/assets/diff1.webp"
            alt="Modern home with a pool"
            class="img-fluid rounded shadow"
          />
        </div>

        <!-- Right Features Section -->
        <div class="col-md-6">
          <div class="row text-center gy-4">
            <!-- Feature 1 -->
            <div class="col-12 col-md-6">
              <div class="d-flex flex-column align-items-center">
                <span class="icon mb-3">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 121.48 122.88"
                    width="40"
                    height="40"
                    fill="black"
                  >
                    <path
                      d="M2.74,19.11L59.65,0.16c0.68-0.23,1.39-0.21,2.02,0.01l0-0.01l57.66,19.75c1.42,0.48,2.28,1.86,2.15,3.29 c0.01,0.07,0.01,0.15,0.01,0.23v67.06h-0.01c0,1.16-0.64,2.28-1.75,2.84l-57.25,29.09c-0.48,0.29-1.05,0.46-1.65,0.46 c-0.64,0-1.23-0.19-1.73-0.51L1.72,92.44c-1.08-0.57-1.71-1.67-1.71-2.82H0V22.27C0,20.66,1.19,19.33,2.74,19.11L2.74,19.11z M15.33,68.24c0-1.22,0.99-2.22,2.22-2.22c1.22,0,2.22,0.99,2.22,2.22v9.03c0,0.07,0,0.15-0.01,0.22c0,0.31,0.04,0.56,0.11,0.75 c0.03,0.06,0.06,0.12,0.12,0.16l5.53,2.57c1.11,0.51,1.59,1.83,1.08,2.93c-0.51,1.11-1.82,1.59-2.93,1.08l-5.66-2.63 c-0.1-0.04-0.2-0.09-0.3-0.16c-0.91-0.57-1.54-1.34-1.93-2.29c-0.31-0.76-0.45-1.6-0.44-2.5l0-0.14V68.24L15.33,68.24z M57.64,114.44V50.3L6.38,27.06V87.7L57.64,114.44L57.64,114.44z M115.1,27.82L64.02,50.33v64.17l51.08-25.96V27.82L115.1,27.82z M60.62,6.53L12.14,22.68l48.71,22.09l48.71-21.47L60.62,6.53L60.62,6.53z"
                    />
                  </svg>
                </span>
                <h5 class="fw-bold">Advanced Technology Integration</h5>
                <p class="mb-0">Embracing cutting-edge technology to deliver smart, efficient, and futuristic living experiences.</p>
              </div>
            </div>

            <!-- Feature 2 -->
            <div class="col-12 col-md-6">
              <div class="d-flex flex-column align-items-center">
                <span class="icon mb-3">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 121.48 122.88"
                    width="40"
                    height="40"
                    fill="black"
                  >
                    <path
                      d="M2.74,19.11L59.65,0.16c0.68-0.23,1.39-0.21,2.02,0.01l0-0.01l57.66,19.75c1.42,0.48,2.28,1.86,2.15,3.29 c0.01,0.07,0.01,0.15,0.01,0.23v67.06h-0.01c0,1.16-0.64,2.28-1.75,2.84l-57.25,29.09c-0.48,0.29-1.05,0.46-1.65,0.46 c-0.64,0-1.23-0.19-1.73-0.51L1.72,92.44c-1.08-0.57-1.71-1.67-1.71-2.82H0V22.27C0,20.66,1.19,19.33,2.74,19.11L2.74,19.11z M15.33,68.24c0-1.22,0.99-2.22,2.22-2.22c1.22,0,2.22,0.99,2.22,2.22v9.03c0,0.07,0,0.15-0.01,0.22c0,0.31,0.04,0.56,0.11,0.75 c0.03,0.06,0.06,0.12,0.12,0.16l5.53,2.57c1.11,0.51,1.59,1.83,1.08,2.93c-0.51,1.11-1.82,1.59-2.93,1.08l-5.66-2.63 c-0.1-0.04-0.2-0.09-0.3-0.16c-0.91-0.57-1.54-1.34-1.93-2.29c-0.31-0.76-0.45-1.6-0.44-2.5l0-0.14V68.24L15.33,68.24z M57.64,114.44V50.3L6.38,27.06V87.7L57.64,114.44L57.64,114.44z M115.1,27.82L64.02,50.33v64.17l51.08-25.96V27.82L115.1,27.82z M60.62,6.53L12.14,22.68l48.71,22.09l48.71-21.47L60.62,6.53L60.62,6.53z"
                    />
                  </svg>
                </span>
                <h5 class="fw-bold">Advanced Technology Integration</h5>
                <p class="mb-0">Embracing cutting-edge technology to deliver smart, efficient, and futuristic living experiences.</p>
              </div>
            </div>
            <div class="col-12 col-md-6">
              <div class="d-flex flex-column align-items-center">
                <span class="icon mb-3">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 121.48 122.88"
                    width="40"
                    height="40"
                    fill="black"
                  >
                    <path
                      d="M2.74,19.11L59.65,0.16c0.68-0.23,1.39-0.21,2.02,0.01l0-0.01l57.66,19.75c1.42,0.48,2.28,1.86,2.15,3.29 c0.01,0.07,0.01,0.15,0.01,0.23v67.06h-0.01c0,1.16-0.64,2.28-1.75,2.84l-57.25,29.09c-0.48,0.29-1.05,0.46-1.65,0.46 c-0.64,0-1.23-0.19-1.73-0.51L1.72,92.44c-1.08-0.57-1.71-1.67-1.71-2.82H0V22.27C0,20.66,1.19,19.33,2.74,19.11L2.74,19.11z M15.33,68.24c0-1.22,0.99-2.22,2.22-2.22c1.22,0,2.22,0.99,2.22,2.22v9.03c0,0.07,0,0.15-0.01,0.22c0,0.31,0.04,0.56,0.11,0.75 c0.03,0.06,0.06,0.12,0.12,0.16l5.53,2.57c1.11,0.51,1.59,1.83,1.08,2.93c-0.51,1.11-1.82,1.59-2.93,1.08l-5.66-2.63 c-0.1-0.04-0.2-0.09-0.3-0.16c-0.91-0.57-1.54-1.34-1.93-2.29c-0.31-0.76-0.45-1.6-0.44-2.5l0-0.14V68.24L15.33,68.24z M57.64,114.44V50.3L6.38,27.06V87.7L57.64,114.44L57.64,114.44z M115.1,27.82L64.02,50.33v64.17l51.08-25.96V27.82L115.1,27.82z M60.62,6.53L12.14,22.68l48.71,22.09l48.71-21.47L60.62,6.53L60.62,6.53z"
                    />
                  </svg>
                </span>
                <h5 class="fw-bold">Advanced Technology Integration</h5>
                <p class="mb-0">Embracing cutting-edge technology to deliver smart, efficient, and futuristic living experiences.</p>
              </div>
            </div>
            <div class="col-12 col-md-6">
              <div class="d-flex flex-column align-items-center">
                <span class="icon mb-3">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 121.48 122.88"
                    width="40"
                    height="40"
                    fill="black"
                  >
                    <path
                      d="M2.74,19.11L59.65,0.16c0.68-0.23,1.39-0.21,2.02,0.01l0-0.01l57.66,19.75c1.42,0.48,2.28,1.86,2.15,3.29 c0.01,0.07,0.01,0.15,0.01,0.23v67.06h-0.01c0,1.16-0.64,2.28-1.75,2.84l-57.25,29.09c-0.48,0.29-1.05,0.46-1.65,0.46 c-0.64,0-1.23-0.19-1.73-0.51L1.72,92.44c-1.08-0.57-1.71-1.67-1.71-2.82H0V22.27C0,20.66,1.19,19.33,2.74,19.11L2.74,19.11z M15.33,68.24c0-1.22,0.99-2.22,2.22-2.22c1.22,0,2.22,0.99,2.22,2.22v9.03c0,0.07,0,0.15-0.01,0.22c0,0.31,0.04,0.56,0.11,0.75 c0.03,0.06,0.06,0.12,0.12,0.16l5.53,2.57c1.11,0.51,1.59,1.83,1.08,2.93c-0.51,1.11-1.82,1.59-2.93,1.08l-5.66-2.63 c-0.1-0.04-0.2-0.09-0.3-0.16c-0.91-0.57-1.54-1.34-1.93-2.29c-0.31-0.76-0.45-1.6-0.44-2.5l0-0.14V68.24L15.33,68.24z M57.64,114.44V50.3L6.38,27.06V87.7L57.64,114.44L57.64,114.44z M115.1,27.82L64.02,50.33v64.17l51.08-25.96V27.82L115.1,27.82z M60.62,6.53L12.14,22.68l48.71,22.09l48.71-21.47L60.62,6.53L60.62,6.53z"
                    />
                  </svg>
                </span>
                <h5 class="fw-bold">Advanced Technology Integration</h5>
                <p class="mb-0">Embracing cutting-edge technology to deliver smart, efficient, and futuristic living experiences.</p>
              </div>
            </div>

            <!-- More features can be added similarly -->

          </div>

          <!-- Learn More Button -->
          <div class="text-center mt-4">
            <a href="#" class="btn btn-outline-primary">Contact Us</a>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script>
export default {
  name: "DifferenceSection",
};
</script>

<style scoped>
.vh-100 {
  height: 100vh !important;
}

.btn-outline-primary {
  background-color: transparent;
  border: 2px solid black;
  color: black;
}

.btn-outline-primary:hover {
  background-color: #c6a266;
  border: #c6a266;
  color: #fff;
}

/* Responsive Media Queries */
@media (max-width: 768px) {
 /* Hide all rows except the first feature */
 .row:not(:first-of-type),
  .col-md-6:not(:first-of-type) {
    display: none;
  }

  /* Center align the visible elements */
  .text-center {
    margin: 0 auto;
    padding: 0 15px;
  }

  .btn-outline-primary {
    width: 100%; /* Full width button for mobile */
  }

  .icon svg {
    width: 30px; /* Smaller icon size */
    height: 30px;
  }
}


@media (min-width: 1200px) {
  .btn-outline-primary {
    max-width: 200px;
  }
}
</style>
